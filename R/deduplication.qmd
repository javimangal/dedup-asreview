...
embase <- embase_raw %>% 
  mutate(
    record_id = record_id, # None available in this dataset, empty column created by load_search 
    ...
    isbn = issn,         # Called issn in original dataset, may have >1 separated by " and " with spaces.
...
scopus <- scopus_raw %>% 
  mutate(
    ...
    number = issue,       # Called issue in original dataset
    abstract = abstract,  # Correct     
    isbn = issn,          # Called issn in original dataset with "(ISSN)" or "(ISBN)" sting after, 
...
openalex <- openalex_raw %>% 
  mutate(
    ...
    number = issue,       # Called issue in original dataset
    abstract = abstract,  # Correct     
    isbn = issn,          # Called issn in original dataset, single issn with no spaces.
    label = label,        # Empty column created by load_search
...
After deduplication, there are a total of `r count(final_dedup)` studies. 

However, the data had remaining unfixed issues such as different name patterns and issn, so it would be good to examine if this changes the results. 
